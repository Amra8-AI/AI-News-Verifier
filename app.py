...
st.markdown("""
<style>
.stApp { background: linear-gradient(135deg, #0f0c29, #302b63, #24243e); color: #f8fafc; }
...
if uploaded_file is not None:
    ...
    if 'text' in uploaded_df.columns and 'label' in uploaded_df.columns:
        ...
        if st.sidebar.button("🚀 Train Model", key="train_btn", help="Train the model with uploaded data"):
...
if batch_file is not None:
    ...
    if 'text' not in batch_df.columns:
        ...
        if st.button("🚀 Analyze All Articles", width='stretch', key="analyze_btn"):
